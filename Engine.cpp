...
Engine::Engine() : window(L"3D Engine"), mouse(), keyboard()
{
	...
	swap_chain_descr.OutputWindow = win;
	...
	if (FAILED(hr))
	{
		MessageBox(win, L"Error generating back buffer for Render target view.", L"ERROR", MB_OK);
	}
	...
	depthBufferDescriptor.Width = win->GetWidth();					//Width of texture in texels.
	depthBufferDescriptor.Height = win->GetHeight();					//Height of texture in texels.
	...
	if (FAILED(hr))
	{
		MessageBox(win, L"Error generating buffer for depth/stencil view.", L"ERROR", MB_OK);
	}

	hr = device->CreateDepthStencilView(pDepthStencilBuffer, nullptr, &depthstencilview);
	if (FAILED(hr))
	{
		MessageBox(win, L"Error generating depth/stencil view.", L"ERROR", MB_OK);
	}
	...
	devicecontext->OMSetDepthStencilState(defaultstencilstate, 0u);

	rendertexture.Initialize(device, win->GetWidth(), win->GetHeight());
	blurtexture.Initialize(device, win->GetWidth(), win->GetHeight());
	...
	defaultviewport.Width = static_cast<float>(win->GetWidth());		//Width is the same as client window width.
	defaultviewport.Height = static_cast<float>(win->GetHeight());		//Height is the same as client window height.
